...
export default function ToolCustomizer(props: any) {
  ...
  const onChangeLineType = (e: any) => {
    ...
    console.log(v)
    dispatch(setToolSetting({ tool: tool, setting: { lineType: v == 'general' ? LineType.General : LineType.Dashed } })
    )
  }
              ...
              }
              {
                !props.hideLineTypePicker && (
                  <Form.Group as={Row}>
                    ...
                    <Col sm="8">
                      <Form.Select onChange={onChangeLineType} value={state.lineType === LineType.General ? 'general' : 'dashed'}>
